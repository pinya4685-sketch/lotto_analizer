...
- **목적**: 1회차부터 최신 1240회차까지의 실제 역대 로또 당첨 데이터를 기반으로 한 AI 퀀트 분석, 필터링 및 번호 예측/추천 하이브리드 모바일 애플리케이션
...
│   │   └── FeatureModals.tsx   # 1~1240회 당첨 이력 조회, 통계 모달 등
│   ├── services/
│   │   └── adMobService.ts     # 실제 AdMob ID 및 테스트 ID 관리
│   ├── data/
│   │   └── lottoData.ts        # 1~1240회 로컬 오프라인 로또 통계 데이터베이스
│   └── utils/
...
2. **연속 미출현 번호 통계 (2번 카테고리)**:
   - 최신 회차(1240회)를 기준으로 각 번호가 몇 주(회차) 동안 미출현했는지 전수 집계.
...
## 11. 사용자 필터 설정(User Filters) 퀀트 엔진 실시간 연동 아키텍처
1. **필터 상태 전역 컨텍스트 영속화 (`LottoContext`)**:
   - `UserFilterSettings` 인터페이스 신설: `includedNums`, `excludedNums`, `filterValues`, `stepToggles`, 수치형 범위 필터(`sumMin`, `sumMax`, `frontSumMin`, `backSumMin`, `oddEvenRatio`, `acMin`, `maxColorCount` 등).
   - `LottoContext`에 `userFilters` 전역 상태 및 `updateUserFilters` 메서드를 바인딩하여, 메인 화면/결과 화면/재추출 전 과정에서 동일한 사용자 필터 일관성 유지.
2. **모달 양방향 동기화 (`FilterSettingModal`)**:
   - 모달 마운트/오픈 시 현재 `userFilters` 값으로 로컬 상태를 복원하여 기존 설정 유지.
   - [필터 설정 저장하기] 터치 시 `updateUserFilters`를 호출하여 퀀트 엔진에 즉시 반영.
   - [🤖 AI 자동 최적화 추천] 실행 시에도 추천된 필터와 제외수를 즉시 컨텍스트에 저장.
3. **퀀트 엔진 100% 무결점 보장 알고리즘 (`quantEngine.ts`)**:
   - **제외수 100% 원천 배제**: 정규 포트폴리오뿐 아니라 역발상(Contrarian) 포트폴리오 및 폴백 풀(Fallback pool)에서도 사용자 제외수를 완전 차단하여 단 1건의 번호 침범도 없음을 수학적으로 보장 (0건).
   - **포함수 100% 무조건 장착**: 무작위 6개 후필터링 방식이 아닌, 후보 생성 시 `includedNums`를 먼저 기본 슬롯으로 확정 배치 후 나머지 슬롯만 유효 풀에서 비복원 추출하여 100% 장착 및 연산 낭비 0 달성.
   - **다단계 충원 안전장치**: 극단적 필터 조건에서도 앱 멈춤 없이 5게임을 채우되, 제외수와 포함수는 어떤 완화 단계에서도 절대 불변 유지.
4. **결과 화면 시각적 확인 배너 (`ResultScreen`)**:
   - 사용자 설정 제외수 및 포함수가 있을 경우, 상단에 `[사용자 맞춤 필터 100% 반영 완료]` 뱃지 바(소각된 제외수 목록, 장착된 포함수 목록)를 실시간 렌더링하여 사용자에게 직관적 신뢰 제공.

---

## 12. V26.1 100% 동적 킬 스위치 (Death Zone) 알고리즘 (하드코딩 배제)
1. **설계 배경**:
   - 매주 새로운 `lottoData`가 누적/업데이트될 때마다 알고리즘이 유동적으로 과열 패턴을 추적할 수 있도록, 특정 숫자 리터럴(예: 13, 2 등)을 일체 코드에 직접 타이핑하지 않고 순수 '변수와 배열의 수학적 패턴'만으로 킬 스위치 작동.
2. **3대 핵심 동적 킬 스위치 구현**:
   - **단일 번호 2주 연속 이월 동적 차단 (Intersection)**:
     - `history[0].nums`(최근 회차)와 `history[1].nums`(직전 회차) 배열을 `filter`와 `includes`로 교차 검증.
     - 공통으로 존재하는 이월 번호가 존재할 경우, 변수에 담아 이번 회차 `deathZone`에 즉시 push.
   - **끝수 과열 2주 연속 동적 킬 스위치 (Frequency Map)**:
     - `history[0]`과 `history[1]` 번호들의 끝수(0~9) 출현 빈도를 Map 객체로 동적 집계.
     - 특정 끝수 `N`이 1주 전 2개 이상 && 최근 회차 2개 이상일 경우 (2주 연속 과열), 1부터 45 중 `(i % 10 === N)`인 모든 번호를 루프 연산으로 `deathZone`에 push.
   - **OMR 세로열 3주 연속 과열 동적 멸종 룰 (Set Intersection)**:
     - 최근 3주치(`history[0]`, `[1]`, `[2]`)에 대해 OMR 세로열 번호(`n % 7 === 0 ? 7 : n % 7`) 집합을 3개의 Set으로 추출.
     - 3개 Set의 3주 연속 교집합 세로열 `C`를 도출하여, 1부터 45 중 해당 열의 모든 번호를 `deathZone`에 push.
3. **콘솔 연동 및 5게임 조합 추출 보장**:
   - `calculateDeathZone` 연산 시 `console.log('V26.1 동적 킬 스위치 알고리즘 연동 완료')` 출력.
   - 도출된 동적 `deathZone` 배열을 기반으로 정규 5게임 및 진성 역발상 5게임이 절대 제외수를 100% 소각한 채 안전하게 추출됨을 시뮬레이션으로 검증 완료.

---

## 13. 1240회차 당첨 데이터 업데이트 내역 (2026-09-05 추첨)
1. **당첨 번호 결과**:
   - **당첨 번호**: `[11, 13, 19, 20, 31, 44]` + **보너스 번호**: `27`
   - **1등 당첨 인원**: 16명
   - **1인당 당첨 금액**: 1,791,817,758원
2. **반영 컴포넌트 및 서비스**:
   - `lottoData.ts` & `lottoData.json`: 1240회차 최신 데이터 등록 (총 1240개 레코드)
   - `lottoApi.ts`: 최신 기본 회차 1240회 및 2026-09-05 날짜, 당첨금/인원 동기화
   - `LottoContext.tsx`: 전역 최신 회차 State 1240회로 갱신
   - `HomeScreen.tsx`: 최신 당첨 배너 1240회차 및 날짜 동적 표기
   - `StatisticsScreen.tsx`: 12개 퀀트 통계 및 Cold Numbers, OMR 패턴에 1240회 데이터 동적 반영
